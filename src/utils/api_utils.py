...
def get_gold_and_log(inds, tokenizer, script_args, metrics):
    ...
    if "contdist" in metrics.keys():
        if len(metrics['contdist'])> 2: 
            rw.likemod = metrics['contdist'][0]
            rw.liketok = metrics['contdist'][1]
        rw.slikemod = metrics['contdist'][-2] # this should do the trick I think?
        rw.sliketok = metrics['contdist'][-1]
        
    allngs = []
    acc = 0
    totnew = 0
    returnscos = []
    # all the indices will start at this value
    base = len(metrics['rscores'])-script_args.inpsize
    # TODO get an assert on the batch size metric to make sure it's ok
    for ind in range(len(metrics['rscores'])-script_args.inpsize, len(metrics['rscores']), 2):
        ...
        if ind in inds:
            ...
            returnscos[jind-base] = max(tmp['rewards'])
            returnscos[kind-base] = min(tmp['rewards'])
